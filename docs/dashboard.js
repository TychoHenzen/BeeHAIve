...
function runAction(payload) {
  return client.runAction(payload);
}

function render() {
  if (!currentState) return;
  renderSummary(currentState.counts || {});
  dashboardOutput.replaceChildren();
  const repositories = currentState.repositories || [];
  if (repositories.length === 0) {
    dashboardOutput.append(element("div", "The service has no linked repositories yet.", "empty"));
  } else {
    repositories.forEach((repository) => dashboardOutput.append(renderRepository(repository)));
  }
...
function renderPbi(repository, pbi) {
  ...
  const controls = element("div", undefined, "actions");
  if (pbi.status === "active" && pbi.run_id) {
...
function renderEscalation(escalation, log) {
  ...
  section.append(element("div", `Tier ${current.current}; ${current.consecutive} consecutive failures`, "muted"));
  if ((log || []).length) {
...
startButton.addEventListener("click", () => runAction({ action: "start" }));
