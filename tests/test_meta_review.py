...
from beehaiive.pbi_creation import PbiCreationRequest, PbiCreationValidationError
...
def test_non_accept_decisions_never_call_pbi_creator(
    stores: tuple[OrchestratorStore, RoutingStore], decision: str
) -> None:
    store, routing = stores
    _seed(store)
    _complete(store)
...
def test_accept_rejects_unresolved_or_conflicting_run_evidence(
    stores: tuple[OrchestratorStore, RoutingStore], evidence_case: str
) -> None:
    store, routing = stores
    _seed(store)
    local_run = _complete(store)
    if evidence_case == "missing":
        ...
        foreign_run = _complete(store, project_id="project-2", repository="other/api")
        ...
        other_run = _complete(store, 2, repository="other/api")
